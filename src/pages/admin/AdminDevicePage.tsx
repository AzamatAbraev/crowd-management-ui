...
const AdminDevicePage: React.FC = () => {
  ...
  const handleDelete = async () => {
    ...
    const ok = await deviceService.deleteDevice(selectedDevice.id);
    if (ok) { setDevices(p => p.filter(d => d.id !== selectedDevice.id)); setSelectedDevice(null); }
    ...
    <div style={{ height: '100vh', backgroundColor: 'var(--bg-dark)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead style={{ position: 'sticky', top: 0, backgroundColor: 'var(--bg-panel)', zIndex: 1, borderBottom: '1px solid var(--border-color)' }}>
                  <tr>
                    {['Device', 'Status', 'Location', 'Last Ping'].map(h => (
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setSelectedDevice(null)}>
          <div className="glass-panel" style={{ width: 480, backgroundColor: 'var(--bg-panel)', display: 'flex', flexDirection: 'column', animation: 'slideIn 0.2s ease', maxHeight: '85vh', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '1.25rem 1.5rem', borderTop: '1px solid var(--border-color)', display: 'flex', gap: '0.6rem' }}>
              ...
              <button onClick={() => setShowDeleteConfirm(true)} style={{ flex: 1, padding: '0.65rem', backgroundColor: 'rgba(239,68,68,0.1)', border: '1px solid var(--status-red)', color: 'var(--status-red)', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 6 }}>
            ...
            <form onSubmit={showAddModal ? handleAddSubmit : handleEditSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                ...
                <select value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value })}
                  ...
                  {['ULTRASONIC_SENSOR', 'CAMERA'].map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      ...
      {showDeleteConfirm && selectedDevice && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100 }}>
          <div className="glass-panel" style={{ width: 400, padding: '2rem', backgroundColor: 'var(--bg-panel)', borderRadius: 16, display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ margin: 0, fontSize: '1rem', fontWeight: 800, color: 'var(--text-main)' }}>Delete Device</h2>
              <button onClick={() => setShowDeleteConfirm(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}><X size={18} /></button>
            </div>
            <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.9rem' }}>
              Delete <strong style={{ color: 'var(--text-main)' }}>{selectedDevice.name || selectedDevice.id}</strong>? This cannot be undone.
            </p>
            <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowDeleteConfirm(false)} style={{ padding: '0.6rem 1.2rem', borderRadius: 8, border: '1px solid var(--border-color)', backgroundColor: 'transparent', color: 'var(--text-muted)', cursor: 'pointer', fontWeight: 600 }}>Cancel</button>
              <button onClick={handleDelete} style={{ padding: '0.6rem 1.2rem', borderRadius: 8, border: 'none', backgroundColor: '#ef4444', color: '#fff', cursor: 'pointer', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>
                <Trash2 size={14} /> Delete
              </button>
            </div>
          </div>
